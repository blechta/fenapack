...
parser.add_argument("--resolution", type=int, default=20,
                    help="mshr resolution")
parser.add_argument("--backend", type=str, default="cgal",
                    choices=["cgal", "tetgen"], help="mshr backend")
parser.add_argument("--debug-mesh", action='store_true',
                    help="debug mesh generation")
...
#    mesh = refine(mesh)
from mshr import *
b0 = Box(Point(-1, 0 , 0), Point(0, 1, 1))
#b1 = Box(Point(0, -1, 0), Point(5, 1, 1))
b1 = Box(Point(0, -1, -1), Point(5, 1, 1))
#resolution = 20
resolution = 20
mesh = generate_mesh(b0 + b1, args.resolution, args.backend)
if args.debug_mesh:
    print("num vertices:", mesh.num_vertices())
    print("num cells:", mesh.num_cells())
    print("radius ratio min: %s, max: %s" % MeshQuality.radius_ratio_min_max(mesh))
    if mesh.geometry().dim() == 3:
        print("dihedral angles min: %s, max: %s" % MeshQuality.dihedral_angles_min_max(mesh))
    exec(MeshQuality.radius_ratio_matplotlib_histogram(mesh))
    exec(MeshQuality.dihedral_angles_matplotlib_histogram(mesh))
    htmlfilename = "mesh.html"
    with open(htmlfilename, "w") as f:
        f.write(X3DOM.html(mesh))
    import webbrowser
...
XDMFFile("ff.xdmf").write(boundary_markers, XDMFFile.Encoding_ASCII)
...
inflow = Expression(("36.0*x[1]*(1.0 - x[1])*x[2]*(1.0 - x[2])", "0.0", "0.0"), degree=4)
...
if args.debug_mesh:
    ...
    bc0.apply(w.vector())
    htmlfilename = "u_bc0.html"
    with open(htmlfilename, "w") as f:
        f.write(X3DOM.html(u))
    webbrowser.open_new_tab(htmlfilename)

    bc1.apply(w.vector())
    htmlfilename = "u_bc1.html"
    with open(htmlfilename, "w") as f:
        f.write(X3DOM.html(u))
    webbrowser.open_new_tab(htmlfilename)

    bc_pcd.apply(w.vector())
    htmlfilename = "p_bc_pcd.html"
    with open(htmlfilename, "w") as f:
        f.write(X3DOM.html(p))
    webbrowser.open_new_tab(htmlfilename)
...
XDMFFile("u.xdmf").write(u, 0.0, XDMFFile.Encoding_HDF5)
XDMFFile("p.xdmf").write(p, 0.0, XDMFFile.Encoding_HDF5)
XDMFFile("uascii.xdmf").write(u, 0.0, XDMFFile.Encoding_ASCII)
XDMFFile("pascii.xdmf").write(p, 0.0, XDMFFile.Encoding_ASCII)
